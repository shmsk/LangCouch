...
const LANG_NAMES: Record<string, string> = { es: "Spanish", pt: "Portuguese", tr: "Turkish", de: "German", fr: "French", it: "Italian" };
...
export function buildInstruction(config: Config, picks: Pick[], grammar: GrammarItem | null = null): string {
  ...
  const lines = [
    `<langcouch>`,
    `Passive language immersion (diglot weave). In the prose of your reply, naturally replace ~${picks.length} common words with ${langName} ones, using ONLY this list:`,
    vocab,
